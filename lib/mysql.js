...
let selectBookById = function(Id){
    let _sql = `select bt.Name,b.Name as BookName,b.Author,b.BookPhoto,b.Price,b.Quantity,b.Describe,DATE_FORMAT(b.PublishTime,'%Y年%m月%d日') as PublishTime,b.PublishCompany,b.IsToCart,b.IsMember,b.IsHot from Books b left join Books_Type bt on bt.Id=b.BookTypeId where b.Id=${Id}`
    return query(_sql);
}
// 通过TypeId查所有图书信息
let selectBooksByTypeId = function(BookTypeId){
    let _sql = `select bt.Name,b.Name as BookName,b.BookPhoto,b.Price,b.Quantity,b.Describe,DATE_FORMAT(b.PublishTime,'%Y年%m月%d日') as PublishTime,b.PublishCompany,b.IsToCart from Books b left join Books_Type bt on bt.Id=b.BookTypeId where b.BookTypeId=${BookTypeId}`
    return query(_sql);
}
// 年排名
let selectYearRank = function(){
    let _sql ="select b.Id,b.Name from Books b where year(b.PublishTime) = year(curdate()) LIMIT 0,7"
    return query(_sql);
}
// 月排名
let selectMonthRank = function(){
    let _sql ="select b.Id,b.Name from Books b where DATE_SUB(CURDATE(), INTERVAL 30 DAY) <= date(b.PublishTime) LIMIT 7,7"
    return query(_sql);
}
// 周排名
let selectWeekRank = function(){
    let _sql ="select b.Id,b.Name from Books b where DATE_SUB(curdate(), INTERVAL 7 DAY) <= date(b.PublishTime) LIMIT 14,7"
    return query(_sql);
}
// 合集
let selectCollection = function(){
    let _sql ="select Id,Name,BookPhoto from Books ORDER BY CollectionNum limit 6"
    return query(_sql);
}
// 寻找热门商品和会员商品
let getGoodList= function(IsHot,IsMember){
...
let insertShopcarts =function(value){
    let _sql ="insert into Shopcarts(BookId,Quantity,TotalPrice) values(?,?,?);"
    return query(_sql,value);
}
// 查询所有购物车
let selectShopcarts =function(userId){
    let _sql =`select b.Name,b.BookPhoto,b.Price,b.Describe,b.Quantity,s.TotalPrice from Shopcarts s left join Books b on s.BookId=b.Id where s.UserId=${userId}`
    return query(_sql);
}
//暴露所有函数接口
module.exports = {
