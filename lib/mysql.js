...
let findUserByName = function(username){
    let _sql = `select Id,Username,Avatar,Email from Users where Username='${username}'`
    return query(_sql);
}
// 通过用户id查询用户信息
let findUserById = function(){
    let _sql = `select Id,Username,Avatar,Email from Users where Id='${id}'`
    return query(_sql);
}
// 更新用户信息
