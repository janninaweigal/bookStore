...
let insertBookType = function(name){
    let _sql = `insert into Books_Type(Name) values('${name}');`
    return query(_sql);
}
// 通过Id查图书信息
let selectBookById = function(Id){
    let _sql = `select b.Id as BookId,bt.Name,b.Name as BookName,b.Isbn,b.Author,b.BookPhoto,b.Price,b.Quantity,b.Describe,DATE_FORMAT(b.PublishTime,'%Y年%m月%d日') as PublishTime,b.PublishCompany,b.IsToCart,b.IsMember,b.IsHot from Books b left join Books_Type bt on bt.Id=b.BookTypeId where b.Id=${Id}`
    return query(_sql);
}
// 添加图书信息
let insertBookInfo = function(value){
    let _sql = `insert into Books(BookTypeId,Author,Name,BookPhoto,Price,Quantity,Describe,PublishCompany) values(?,?,?,?,?,?,?,?);`
    return query(_sql,value);
}
// 编辑图书信息
let updateBookInfo = function(value){
    let _sql = `update Books set Author=?,Name=?,BookPhoto=?,Price=?,Quantity=?,Describe=?,PublishCompany=? where Id=?`
    return query(_sql,value);
}
// 通过TypeId查所有图书信息
let selectBooksByTypeId = function(BookTypeId){
    let _sql = `select b.Id as BookId,bt.Name,b.Name as BookName,b.BookPhoto,b.Price,b.Quantity,b.Describe,DATE_FORMAT(b.PublishTime,'%Y年%m月%d日') as PublishTime,b.PublishCompany,b.IsToCart from Books b left join Books_Type bt on bt.Id=b.BookTypeId where b.BookTypeId=${BookTypeId}`
    return query(_sql);
}
// 所有图书信息
let selectBookList = function(globalName,pageNo,pageSize){
    let _sql = `select b.Author,b.Name,bt.Name as typeName,b.Quantity,b.Price,b.Describe,b.Isbn,b.BookPhoto,b.PublishCompany,DATE_FORMAT(b.PublishTime,'%Y年%m月%d日') as PublishTime from Books b LEFT JOIN Books_Type bt on b.BookTypeId=bt.Id where b.Name like '%${globalName}%' or b.Author like '%${globalName}%' or b.PublishCompany like '%${globalName}%'  limit ${pageNo},${pageSize}`
    return query(_sql);
}
// 所有图书信息长度
...
let updateShopcartsQuantity =function(value){
    let _sql ="update Shopcarts set Quantity=?,TotalPrice=? where Id=?"
    return query(_sql,value);
}
// 删除购物车
...
let selectShopcarts =function(userId){
    let _sql =`select s.Id as cartId,b.Name,b.BookPhoto,b.Price,b.Describe,s.Quantity,s.TotalPrice from Shopcarts s left join Books b on s.BookId=b.Id where s.UserId=${userId}`
    return query(_sql);
}
